...
float vel_x=0, vel_y=0,contadort=1;
...
const mrs::Velocity2d &PairingRobot::action(std::vector<mrs::RobotPtr> &neigh) {
    ...
    float random;
    float modVel,totalDistance;
    int emparejado,error;
...
//if(neigh.size()==0)
    ...
    //(800.0*40.0);
    float error_t;
    error_t=error/ciclos*100;
    if (neigh.size() > 0) {
        std::cout << "Distancia media: " << (totalDistance*10/(40)) << std::endl;
    }
    std::cout << "Parejas emparejadas en total " << emparejado/800 << std::endl;
    std::cout << "Veces demasiado error " << (error_t) << "%"<<std::endl;
